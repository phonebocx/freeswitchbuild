...
# This should auto-update to whatever python version is installed
PINC=$(echo /usr/include/python3.??)

if [ ! -d "$PINC" ]; then
	echo "Could not find python include dir"
	echo "PINC is $PINC"
	ls -ald /usr/include/py*
	exit 9
fi
